...
/*通过发送邮件的方式将相关的性能信息或者警告信息发送给客户（包括附件）*可能会出现对方收不到邮件的情况，这是因为发送的邮件有时候会被当做垃圾邮件从而被拒收*将邮件抄送自己一份就可以解决这个问题*/
...
public class SendMail {
	...
	private static MimeMessage createEmail(Session session, String name, String mail, String[] source)
		...
		email.setRecipient(MimeMessage.RecipientType.TO, new InternetAddress(mail, "用户", "UTF-8"));
		email.setRecipient(MimeMessage.RecipientType.CC, new InternetAddress(sendPersonAccount, "用户", "UTF-8"));
		// 标题
		email.setSubject("性能异常警告信息", "UTF-8");
		// 正文
		// 创建图片节点
		...
		String attachmentSource = "C:\\Users\\王国伟\\Desktop\\毕设\\excel\\wgw_cpuException.xls";
		MimeBodyPart attachment = new MimeBodyPart();
		DataHandler dhHandler = new DataHandler(new FileDataSource(source[0]));
		attachment.setDataHandler(dhHandler);
		// 设置附件的文件名
		attachment.setFileName(MimeUtility.encodeText("内存异常信息表.xls", "gbk", null));

		MimeBodyPart text3 = new MimeBodyPart();
		text3.setContent("使用JavaMail创建的带附件的邮件", "text/html;charset=UTF-8");
		MimeBodyPart attachment2 = new MimeBodyPart();
		DataHandler dhHandler2 = new DataHandler(new FileDataSource(source[1]));
		attachment2.setDataHandler(dhHandler2);
		// 设置附件的文件名
		attachment2.setFileName(MimeUtility.encodeText("cpu异常信息表.xls", "gbk", null));
