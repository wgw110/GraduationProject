...
public class BrokenLineTest extends ApplicationFrame {
	 ...
	 * 同一个图例的数据的添加顺序影响最终的显示；不同图例的数据的添加顺序不影响最终的显示。      * @return     
	 */
	private static CategoryDataset createDataset() {
		 ...
		 * 
		 *         //图例2         defaultcategorydataset.addValue(712.0,
		 * "Second", "1001.0");          defaultcategorydataset.addValue(1204.0,
		 * "Second", "1001.1");         defaultcategorydataset.addValue(1820.0,
		 * "Second", "1001.2");         /
		 */
		// *
		// 添加数据方法2
		...
		// 图例1，40个数据
		for (int i = 0; i < 40; i++) {
			defaultcategorydataset.addValue(random.nextInt(100000), "First", String.valueOf(1000 + i));
		}
		// 图例2，40个数据
		for (int i = 0; i < 40; i++) {
			defaultcategorydataset.addValue(random.nextInt(100000), "Second", String.valueOf(1000 + i));
		}
		// */        
	...
	private static JFreeChart createChart(CategoryDataset categorydataset) {
		JFreeChart jfreechart = ChartFactory.createLineChart("jfreechart test", // 图表标题
				"X", // 主轴标签（x轴）
				"Y", // 范围轴标签（y轴）
				categorydataset, // 数据集
		...
		lineandshaperenderer.setBaseItemLabelGenerator(new StandardCategoryItemLabelGenerator("{2}", decimalformat1));//  
																														// 设置数据项标签的生成器
	...
	public static JPanel createDemoPanel() {
		...
		try {
			ChartUtilities.saveChartAsJPEG(new File("picture\\LineChartDemo1.jpg"), // 文件保存物理路径包括路径和文件名
					//     1.0f,    //图片质量 ，0.0f~1.0f
					jfreechart, // 图表对象
	...
	public static void main(String[] strings) {
		BrokenLineTest linechartdemo1 = new BrokenLineTest("JFreeChart - Line Chart Demo 1");
		linechartdemo1.pack();
